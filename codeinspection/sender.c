 ...
 * ------------------------------------------------------------- functions --
 */
void startSender(int parameter);
...
int main(int argc, char *argv[])
{
	...
	if(doit == 1){
		startSender(parameter);
		/* bei erfolgreichem Beenden räummt der empfaenger auf */
		(void)fflush(stdout);
		return 0;
	} 
 ...
 * Liest aus stdin und schreibt in Ringbuffer des Shared Memorys
 * 
 *
...
void startSender(int parameter)
{
	...
	while(1)
	{
		...
		/* schreiben auf das shared memory */
		*a = c;
		...
		/* ringpuffer weiterzählen */
		a++;
		/* position im ringpuffer merken */
		i++;

		/* wenn position im puffer auf max = parameter ist dann position wieder auf null stellen */
		if(i == parameter)
		{	
			a = shmptr;
			i=0;
		...
		/* wenn der eingelesen character EOF ist dann das Programm beenden, input fertig eingelesen */
		if(c == EOF)
			break;
	}
	
	errno = 0;
	/* entfernt den shared memory aus dem Programm */
	if (shmdt(shmptr) == -1)
